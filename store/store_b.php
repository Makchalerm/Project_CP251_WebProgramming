...
<body>
    ...
    $ID2 = $_SESSION['ID'];
    $sql = "SELECT * FROM ((store_detail INNER JOIN register ON register.regID = store_detail.regID) INNER JOIN users ON store_detail.ID = users.ID) WHERE register.ID = '$ID2'";
    ...
    <div class="w3-container w3-light-gray">
        <div class="container">
            <form id="searchfrom">
                <br><label for="found" class="w3-center w3-large w3-left w3-cell-middle"><?php echo $num ?> History to contact People</label>
            </form>
            <br>
            <hr>
        </div>
        ...
        $sql = "SELECT * FROM ((store_detail INNER JOIN register ON register.regID = store_detail.regID) INNER JOIN users ON store_detail.ID = users.ID) WHERE register.ID = '$ID2' LIMIT {$frompage}, {$perpageview}";
        $query = mysqli_query($conn, $sql);
        ?>
            ...
            <div class="container">
                <div class="w3-card-4 w3-round-large" id="list-data">
                    ...
                    <div class="w3-container">
                        ...
                        <p>Tel: <?php echo $result['tel']; ?></p>
                        <hr>
                    </div>
                    <button onclick="window.location.href='delete_b.php?detail_ID=<?php echo $result['detail_ID']; ?>' " class="w3-button w3-block w3-dark-grey w3-round-large">Delete</button>
                </div>
            </div>
            <br>
        <?php } ?>
            ...
            <div class="w3-center w3-large">
                <ul class="pagination">
                    <?php for ($i = 1; $i <= $pagecount; $i++) { ?>
                        <a href="store_b.php?pageno=<?php echo $i; ?>"> <?php echo $i; ?></a>
                    <?php } ?>
                </ul>
            </div>
        </div>
